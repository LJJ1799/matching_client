...
from obj_sample_and_autolabel import sample_and_label_alternative, sample_and_label_parallel
import slice
...
class LookupTable():
    ...
    def __init__(self, 
        ...
        #     if os.path.splitext(component)[-1] == '.obj':
        #         old_name = os.path.splitext(component)[0]
        #         # if not old_name == component:
        #         #     os.rename(component,os.path.join(self.path_models,component,component+'.obj'))
        #         file_data = ''
        ...
        #             f.write(file_data)
        #     if os.path.splitext(component)[-1] == '.xml':
    ...
    def make(self, free_cores = 1):
        ...
        if self.profile and not self.skip_sampling:
            ...
            folders = [folder for folder in folders if os.path.isdir(folder)]    # remove non-folders
        ...
        if self.decrease_lib:
            ...
            slice.decrease_lib(self.path_data, self.path_data, path_welding_zone, label_dict_r)
        else:
            print('not reducing stuff :)')
            slice.decrease_lib_dummy(self.path_data, self.path_data, path_welding_zone, label_dict_r)
