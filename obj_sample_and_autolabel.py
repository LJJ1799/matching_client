...
PATH_COMP = '../data/source/models'
PATH_XYZ = '../data/source/unlabeled_pc'
PATH_PCD = '../data/source/labeled_pc'
    ...
    for folder in folders:
        try:
            if sample_alternative:
                sample_and_label_alternative(folder, path_pcd, path_xyz, label_dict, class_dict, density)
            else:
                sample_and_label(folder, path_pcd, path_xyz, label_dict, class_dict, density)
...
def sample_and_label_alternative(path, path_pcd, path_xyz, label_dict, class_dict, density=40):
    ...
    for file in files:
        if os.path.splitext(file)[1] == '.obj':
            ...
            if np.asarray(mesh.triangles).shape[0] > 1:
                label = label_dict[class_dict[file]]
                # get number of points according to surface area
                number_points = int(mesh.get_surface_area()/density)
                if number_points <= 0:
                    number_points = 1000
