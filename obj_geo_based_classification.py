...
from utils.math_util import get_projections_of_triangle, get_angle
from utils.foundation import show_obj
...
class PFE():
    ...
    def split(self, path_file:str):
        ...
        with open(path_file,'r') as f:
            ...
            os.makedirs(os.path.join(self.path_split,namestr),exist_ok=True)
            # prefix of single disassembled part
            outstr = os.path.join(self.path_split, namestr) + '/' + namestr + '_'
            ...
            shutil.copy(os.path.join(self.path_models,namestr+'.mtl'),
                                  os.path.join( self.path_split, namestr))
            # os.system('copy %s %s'%(os.path.join(self.path_models,namestr,namestr+'.mtl'),
            #                       os.path.join( self.path_split, namestr)))
    ...
    def label_special_cases(features, files):
        ...
        for case in list:
            files.remove(case)
            case = case.rstrip()
            os.system('copy %s %s'%(case, '../data/train/label_temp_folder/special_cases'))
    ...
    def label(self, feats= None, parallel = True):
        ...
        if feats is None:
            ...
            files = open(os.path.join(self.path_split, 'all_parts.txt'), 'r').readlines()
            ...
            # save features
            np.save(os.path.join(self.path_split, 'features.npy'),features)
        else:
            features = feats
        ...
        for i in range(n_clusters):
            ...
            os.makedirs(self.path_label_temp+'/'+str(i),exist_ok=True)
            # os.system('mkdir -p %s'%self.path_label_temp+'/'+str(i))
            idx = np.where(y_pred==i)
            print('idx',idx)
            for j in range(len(idx[0])):
                labels_dict[files[idx[0][j]].strip()] = i
                shutil.copy(files[idx[0][j]].strip(), self.path_label_temp+'/'+str(i))
                # os.system('copy %s %s'%(files[idx[0][j]].strip(), self.path_label_temp+'/'+str(i)))
    ...
    def relabel(self, n, parallelize = True):
        ...
        for file in files:
            ...
            features.append(feature)
        features = np.asarray(features)
        features = np.load(os.path.join(self.path_split,'features.npy'))
...
def main(pfe):
    ...
    for key in class_dict:
        ...
        shutil.copy(key, class_dir)
        # os.system('copy %s %s'%(key, class_dir))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_clusters', type=int, required=False, default=8, help='pre-defined the amount of clusters, skips all user inputs.')
    n_clusters = parser.parse_args().n_clusters
    pfe = PFE(path_models=os.path.join(ROOT, 'data', 'source', 'models'),
              path_split=os.path.join(ROOT, 'data', 'source', 'split'),
              path_label_temp=os.path.join(ROOT, 'data', 'source', 'label_temp_folder'),
              path_classes=os.path.join(ROOT, 'data', 'source', 'parts_classification'),
              parallel=True,
